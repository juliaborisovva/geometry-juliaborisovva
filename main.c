...
вывести ошибку*/
Type determine_figure (char** cursor_start, size_t length_of_type, char* type_circle, char* type_triangle) {
...
Bool parse_circle(char **cursor_start, char **cursor_end, int* counter) {
    ...
    *cursor_start = *cursor_end;
    ...
    // можно поставить !!implementation и будет работать код, не писать == FAILURE
...
int main()
{
    ...
    while ((fgets(input, 70, stdin))) {
        ...
        figure = determine_figure(&cursor_start, length_of_type, type_circle, type_triangle);
        if (figure == UNKNOWN) {
            continue;
        }
